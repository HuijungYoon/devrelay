...
- npm 배포 버전: **0.10.0** (두 패키지 동일 버전으로 맞춤). 플러그인 pin은 publish 후에 올립니다 (§8)
...
스킬 17개 — 4개 플러그인 모두 `plugins/<client>/skills/<name>/SKILL.md`:
`help` · `test-connection` · `list-projects` · `my-issues` · `issue` · `create-issue` · `update-issue` · `add-comment` · `add-attachment` · `update-status` · `relate-issue` · `subtask` · `log-time` · `read-attachment` · `search-issues` · `bulk-status` · `weekly-report`
