...
**현재 배포 버전: `0.10.0`** (`redmine-devrelay` / `redmine-devrelay-client`)
...
- **PreToolUse 가드**가 Claude Code 플러그인과 함께 설치됩니다 — Redmine에 직접
  POST/PUT/DELETE 하는 셸 명령·스크립트 작성을 거부해 confirm 게이트를 우회할 수 없게 합니다.
  조회는 막지 않습니다 (`plugins/claude-code/hooks/`)
- `scripts/redmine-call.mjs`로 실제 서버를 거친 단일 도구 호출 — MCP 도구가 없는 세션도
  dry-run → previewToken → confirm을 그대로 탑니다
...
| `/bulk-status` | 여러 일감 일괄 변경 — 상태 일괄, 또는 건별 필드 (표 미리보기 한 번 → 확인) |
| `/log-time` | 작업시간 기록 (dry-run → 확인)·조회 |
| `/read-attachment` | 첨부 내려받아 읽기 (읽기 전용) |
| `/weekly-report` | 주간보고 초안 — 표 형식, 이번 주 내 일감·댓글·작업시간·다음 주 마감 (읽기 전용) |
...
Claude Code / Codex / Cursor / Antigravity  (플러그인 + 스킬)
        │ MCP STDIO
        ▼
   redmine-devrelay@0.10.0   (도구 스키마, STDIO, npm)
        │
        ▼
   redmine-devrelay-client@0.10.0  (REST, 인증, HTML 포맷, 쓰기)
        │ HTTPS 또는 사설 IP HTTP
        ▼
   Redmine REST API
```
...
| 경로 | 역할 |
| --- | --- |
| `packages/redmine-client` | npm: `redmine-devrelay-client@0.10.0` |
| `packages/redmine-mcp` | npm: `redmine-devrelay@0.10.0` |
...
| `redmine_bulk_update_issue` | 1~50개 일감을 한 번의 미리보기로 — `issues[]`는 건별 필드, `common`은 공통. 같은 필드면 행이 이김 |
...
또는 `plugins/cursor/mcp.json` / MCP 설정에서 `npx -y redmine-devrelay@0.10.0` 연결 후 `REDMINE_URL` / `REDMINE_API_KEY` 설정.
...
MIT · npm: [`redmine-devrelay@0.10.0`](https://www.npmjs.com/package/redmine-devrelay), [`redmine-devrelay-client@0.10.0`](https://www.npmjs.com/package/redmine-devrelay-client)
