...
Codex·Claude Code·Cursor에서 **자연어와 슬래시 명령**으로 Redmine 이슈를 조회할 수 있게 해주는 로컬 MCP 연동 프로젝트입니다.

에이전트가 Redmine REST API를 직접 만지지 않고, 공통 MCP 서버(`redmine-devrelay`)를 통해 안전하게 읽기 전용으로 접근합니다. **Phase 1은 조회만** 지원하며, 이슈 생성·수정·댓글 등록 같은 쓰기는 포함하지 않습니다.
...
```
Claude Code / Codex / Cursor  (얇은 플러그인 + 스킬)
        │ MCP STDIO
        ▼
   redmine-devrelay   (도구 스키마, STDIO 서버, npm)
        │
        ▼
   redmine-devrelay-client  (REST, 인증, 페이지네이션, 오류 표준화)
        │ HTTPS (또는 allowlist된 사설 http)
        ▼
   Redmine REST API
```
...
| 경로 | 역할 |
| --- | --- |
| `packages/redmine-client` | npm: `redmine-devrelay-client` |
| `packages/redmine-mcp` | npm: `redmine-devrelay` (조회 도구 4개) |
| `plugins/cursor` | Cursor Marketplace 플러그인 (`/add-plugin redmine-devrelay`) |
...
사설망 HTTP(예: `http://192.168.1.20/redmine`)도 `REDMINE_ALLOWED_HOSTS`에 해당 호스트를 넣으면 허용됩니다.
...
그 전까지는 MCP 딥링크 또는 `plugins/cursor/mcp.json` / 로컬 `npx redmine-devrelay@0.1.0`으로 연결합니다. 설치 후 `REDMINE_URL` / `REDMINE_API_KEY` / `REDMINE_ALLOWED_HOSTS`를 설정하세요.

제출: https://cursor.com/marketplace/publish

### 4. Claude Code에서 쓰기

`plugins/claude-code/.mcp.json`은 `npx -y redmine-devrelay@0.1.0`을 사용합니다. 로컬 빌드를 쓰려면:
...
```
packages/redmine-client/   # npm: redmine-devrelay-client
packages/redmine-mcp/      # npm: redmine-devrelay
plugins/cursor/            # Cursor Marketplace 플러그인
...
| [plugins/cursor/README.md](plugins/cursor/README.md) | Cursor `/add-plugin` 제출 안내 |

## 라이선스 / 배포

공개 npm: [`redmine-devrelay@0.1.0`](https://www.npmjs.com/package/redmine-devrelay), [`redmine-devrelay-client@0.1.0`](https://www.npmjs.com/package/redmine-devrelay-client).
