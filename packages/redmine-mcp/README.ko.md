...
| `redmine_bulk_update_issue` | 1~50개 일감을 한 번의 미리보기로. `issues[]`에 건별 필드(`doneRatio`·`statusId`·`assignedTo`·`notes` …), `common`은 모두에게 적용되고 같은 필드면 행이 이김. dry-run → `rows[]`(제목, `changes[]`, `unchanged`/`error`)와 `previewToken` 하나, confirm은 한 건씩 적용해 `updated`/`skipped`/`failed`. 제목·설명·상위일감은 단건 도구에서만 |
