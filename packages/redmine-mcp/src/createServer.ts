...
import { handleSearchText } from "./tools/search.js";
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      ...
      switch (toolName) {
        ...
        case "redmine_search_text": {
          const parsed = safeParseSearchText(req.params.arguments);
          if (!parsed.success) throw validationError(parsed.error.message);
          result = await handleSearchText(client, parsed.data);
          break;
        }
        ...
        case "redmine_list_time_entries": {
          const parsed = safeParseListTimeEntries(req.params.arguments);
          if (!parsed.success) throw validationError(parsed.error.message);
          result = await handleListTimeEntries(client, parsed.data);
          break;
        }
        ...
        case "redmine_bulk_update_issue": {
          const parsed = safeParseBulkUpdateIssue(req.params.arguments);
          if (!parsed.success) throw validationError(parsed.error.message);
          result = await handleBulkUpdateIssue(client, parsed.data);
          break;
        }
