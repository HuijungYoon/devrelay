...
IDE + CLI plugin for Google Antigravity (`agy`). MCP: `redmine-devrelay@0.8.0` (read + write, dry-run confirm gate).
...
`mcp_config.json` pins `npx -y redmine-devrelay@0.8.0` and passes `REDMINE_*`.
...
| `/redmine:weekly-report` | Weekly report draft from my issues, journals, time entries (read-only) |
