...
IDE + CLI plugin for Google Antigravity (`agy`). MCP: `redmine-devrelay@0.10.0` (read + write, dry-run confirm gate).
...
`mcp_config.json` pins `npx -y redmine-devrelay@0.10.0` and passes `REDMINE_*`.
...
| `/redmine:bulk-status` | Several issues at once — same status, or per-issue fields — one table preview, one confirm |
...
| `/redmine:weekly-report` | Weekly report draft as tables from my issues, journals, time entries (read-only) |
