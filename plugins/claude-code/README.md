...
Redmine read/write via `redmine-devrelay@0.10.0` (dry-run → confirm → apply).
...
`.mcp.json` uses `npx -y redmine-devrelay@0.10.0`.

## Write guard

This plugin also installs a PreToolUse hook (`hooks/`) that refuses shell commands
and file writes which POST/PUT/DELETE to Redmine directly — those bypass the
dry-run confirm gate. Reads are untouched. Run `/reload-plugins` after updating.
...
| `/redmine-devrelay:bulk-status` | Several issues at once — same status, or per-issue fields — one table preview, one confirm |
