...
| `/redmine-devrelay:search-issues` | 일감 검색 (마감일·유형·상태·사람 필터, 전문 검색) |
...
| `/redmine-devrelay:bulk-status` | 여러 일감 일괄 변경 — 같은 상태로, 또는 진행률·댓글 등 건별 (표 미리보기 한 번, dry-run 후 확인) |
| `/redmine-devrelay:relate-issue` | 연결된 일감 추가·수정·삭제 (dry-run 후 확인) |
| `/redmine-devrelay:subtask` | 하위일감 생성·부모 변경·연결 해제 (dry-run 후 확인) |
| `/redmine-devrelay:log-time` | 작업시간 기록 (dry-run 후 확인)·조회 |
| `/redmine-devrelay:read-attachment` | 첨부 내려받아 읽기 (읽기 전용) |
| `/redmine-devrelay:weekly-report` | 주간보고 초안 — 표 형식 (이번 주 내 일감·댓글·작업시간·다음 주 마감, 읽기 전용) |
