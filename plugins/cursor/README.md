...
**MCP 패키지:** `redmine-devrelay@0.10.0` (조회 + 쓰기, dry-run 확인 게이트)

## Components

- **MCP:** `npx -y redmine-devrelay@0.10.0`
...
| `/bulk-status` | 여러 일감 일괄 변경 — 상태 일괄, 또는 건별 필드 (표 미리보기 → 확인) |
...
Plugin path: `plugins/cursor` · `mcp.json`은 `redmine-devrelay@0.10.0`를 가리킵니다.
